...
export class RenderEngine {
	...
	public path(path: Point[], width = 1, style: string | CanvasGradient | CanvasPattern = 'black') {
		this.context.lineWidth = width;
		this.context.strokeStyle = style;

		const [fx, fy] = this.spaceToCanvas(path[0]);
		this.context.beginPath();
		this.context.moveTo(fx, fy);
		path.forEach((pt) => {
			const [px, py] = this.spaceToCanvas(pt);
	...
	public fillPath(path: Point[], width = 1, style: string | CanvasGradient | CanvasPattern = 'black') {
		this.context.lineWidth = width;
		this.context.strokeStyle = style;
